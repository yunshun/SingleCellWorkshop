...
```{r HVG_plot, fig.cap="A mean-variance plot where top 1500 HVGs are highlighted in red and top 50 HVGs are labelled.", fig.asp=0.8}
...
```{r UMAP, fig.cap="UMAP visualization where cells are coloured by cell cluster.", fig.asp=0.85}
...
```{r UMAP_Sample, fig.cap="UMAP visualization of the integrated data where cells are coloured by patient."}
...
```{r UMAP_Cluster, fig.cap="UMAP visualization of the integrated data where cells are coloured by cluster."}
...
```{r SigScorePlot2, fig.cap="Signature scores for the integrated data.", fig.wide=TRUE, fig.asp=0.8}
...
```{r TernaryPlot2, fig.cap="Violin plots of signature scores for the integrated data.", fig.small=TRUE, fig.asp=1}
...
Here, we use `FindAllMarkers()` to obtain markers for every cluster compared to all remaining cells.
Same as bofore, we are only interested in the positive markers.
```{r AllMarkersNormEpi}
NormEpi.markers <- FindAllMarkers(NormEpi, only.pos = TRUE)
```

The top 3 marker genes for each of cluster are listed below.
...
We highlight the expression levels of these markers genes in the UMAP plots for visualization.
